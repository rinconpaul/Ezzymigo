...
import { saveRelationships } from '../server/relationships/index.js';
...
async function seedTestFixtures() {
  ...
  const plumberMemory = {
    ...
    interpretation: {
      ...
      retrieval_cues: ['plumber quote', 'gutters', 'Dave plumber quote', 'gutter cleaning price'],
      relationships: [{ person: 'Dave', role: 'plumber', is_active: true }],
      resurfacing: { mode: 'contextual', timing: 'Contextual / On retrieval' },
    },
  };
  ...
  // Insert Relationships (Active Plumber Dave, Deactivated Mechanic Bill)
  await saveRelationships([
    { person: 'Dave', role: 'plumber', is_active: true },
  ]);

  // Seed Deactivated Mechanic Bill directly
  ...
  await executeBunnySql([
    {
      ...
      args: ['rel_bill_mechanic', 'Bill', 'mechanic', 'mechanic', 0, nowIso]
    }
  ]);
...
async function cleanupTestFixtures() {
  ...
  await executeBunnySql([
    ...
    { sql: `DELETE FROM user_relationships WHERE person IN ('Dave', 'Bill');` },
...
async function runAskParityHarness() {
  ...
  // --- Archetype 2: Relationship-aware retrieval ---
  {
    ...
    const resp = await queryAsk(query);
    const ansLower = (resp.answer || '').toLowerCase();
    const hasDave = ansLower.includes('dave');
    const passed = hasDave && !resp.is_out_of_scope;

    results.push({
      archetype: 'Relationship-aware retrieval (Identity)',
      query,
      passed,
      notes: `Expected Dave identified as plumber.`,
      actualAnswer: resp.answer,
      actualMemoryIds: resp.memory_ids,
      isOutOfScope: resp.is_out_of_scope,
    });
...

  {
    ...
    const hasDaveOrPlumber = ansLower.includes('dave') || ansLower.includes('plumber') || ansLower.includes('gutter');
    const hasMemId = Array.isArray(resp.memory_ids) && resp.memory_ids.includes('test_ask_mem_plumber_quote');
    const passed = hasPrice && hasDaveOrPlumber && hasMemId && !resp.is_out_of_scope;
  ...
  // --- Archetype 11: Deleted/forgotten knowledge suppression ---
  {
    ...
    const resp = await queryAsk(query);
    const ansLower = (resp.answer || '').toLowerCase();
    const doesNotClaimBill = !ansLower.includes('bill is your mechanic') && !ansLower.includes('your mechanic is bill');
    const mentionsNoRecord = ansLower.includes("don't have") || ansLower.includes("couldn't find") || ansLower.includes("no record");
    const passed = doesNotClaimBill && mentionsNoRecord;

    results.push({
      archetype: 'Deleted/forgotten knowledge suppression',
      query,
      passed,
      notes: `Expected no mention of deactivated Bill as mechanic.`,
      actualAnswer: resp.answer,
      actualMemoryIds: resp.memory_ids,
      isOutOfScope: resp.is_out_of_scope,
    });
