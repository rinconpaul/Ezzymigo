...
async function main() {
  console.log(`\n================================================================================`);
  console.log(`  ARCHITECTURE D — STAGE 2 TORTURE EXAM: EXACT-SET CONTEXT COMPLETION`);
  console.log(`================================================================================\n`);

  await initBunnyDb();

  const corpusRaw = fs.readFileSync('multilingual_torture_fixtures_v1.json', 'utf8');
  const corpus: CorpusData = JSON.parse(corpusRaw);

  const BATCH_SIZE = 11;
  const totalFixtures = corpus.fixtures.length;
  const numBatches = Math.ceil(totalFixtures / BATCH_SIZE);
  ...
  for (let batchNum = 1; batchNum <= numBatches; batchNum++) {
    ...
    const endIdx = Math.min(startIdx + BATCH_SIZE, totalFixtures);
    const batchFixtures = corpus.fixtures.slice(startIdx, endIdx);

    console.log(`--- RUNNING BATCH ${batchNum}/${numBatches} (Fixtures ${startIdx + 1} to ${endIdx}) ---`);
    ...
    for (let i = startIdx; i < endIdx; i++) {
      ...
      if (globalInfraHalt) {
        allResults.push({
          ...
          passed: false,
          is_inconclusive: true,
          failure_reason: 'Skipped due to prior infrastructure/quota failure in run',
          defect_classification: 'INCONCLUSIVE_INFRA_FAILURE',
          embedding_call_attempted: false,
          embedding_call_succeeded: false,
          embedding_error_code: 'INFRA_HALT',
          vector_sql_executed: false,
    ...
    fs.writeFileSync(`scripts/torture-stage2-batch${batchNum}-results.json`, JSON.stringify(batchResults, null, 2));

    const conclusive = allResults.filter(r => !r.is_inconclusive);
    const passedConclusive = conclusive.filter(r => r.passed).length;
    fs.writeFileSync('scripts/torture-stage2-results.json', JSON.stringify({
  ...
  // 2. Negative Controls Validation
  ...
  for (const nc of corpus.negative_control_mutations) {
    ...
    // A mutated poisoned fixture is DETECTED when the retrieval evaluation fails (passes === false)
    // because Architecture D did not satisfy the poisoned ground-truth requirement.
  ...
  fs.writeFileSync('scripts/torture-stage2-negative-controls.json', JSON.stringify(ncSummary, null, 2));
  ...
  fs.writeFileSync('scripts/torture-stage2-results.json', JSON.stringify(stage2Report, null, 2));
  fs.writeFileSync('scripts/torture-stage2-negative-controls.json', JSON.stringify(stage2Report.negative_controls, null, 2));
  console.log(`\nSaved final audited results to scripts/torture-stage2-results.json and scripts/torture-stage2-negative-controls.json`);

  console.log(`\n================================================================================`);
  console.log(`  STAGE 2 AUDITED SCORE: ${passedCount} / ${validEvaluated.length} conclusive cases (${stage2Report.pass_rate_pct}%)`);
