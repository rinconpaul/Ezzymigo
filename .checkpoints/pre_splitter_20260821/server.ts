...
Analyze and interpret this user capture:
"${trimmedText}"`,
        config: {
...
CRITICAL RULES:
1. MULTI-INTENTION DECOMPOSITION:
   - Every independently useful task, intention, reminder, question, idea, or fact in the user's capture MUST become its own memory object in the "memories" array.

2. EXPLICIT CLASSIFICATION (REMINDER vs. FACT / MEMORY):
   - TEMPORAL INTENT (kind: "reminder"):
     - If the user's capture contains any temporal intent or time expression (e.g. "remind me in 10 minutes", "tomorrow morning at 4am", "dentist Tuesday at 2pm", "remind me Saturday to buy dog food"), classify as "reminder" (or "event" if an appointment).
     - Set resurfacing.mode to "date_based" and resurfacing.timing to the requested time expression.
   - NON-TEMPORAL INTENT (kind: "fact"):
     - If the user's capture does NOT contain a time expression (e.g., general knowledge, personal notes, preferences, facts, observations like "Spare 9V batteries are in the laundry cupboard", "Barb's favourite flower is jasmine", "The wifi password is blue-mountain-99"), classify kind as "fact" (or "task" only if it is an unscheduled action item, or "idea" if a concept).
     - For non-temporal memories and facts: Set resurfacing.mode to "contextual" and resurfacing.timing to "Contextual / On retrieval".

3. ORIGINAL TIME EXPRESSION & STRICT PROHIBITION ON INFERRED / CONTEXTUAL PHRASES:
   - "original_time_expression" MUST ONLY contain explicit literal clock, calendar, or relative duration temporal wording actually supplied by the user (e.g. "tomorrow morning", "in 10 minutes", "Saturday 9am", "Monday at 7pm").
...
4. MANDATORY CONTEXTS & RETRIEVAL CUES (CANNOT BE EMPTY):
   - "contexts": MUST be a non-empty array with 1 to 5 useful circumstances, life domains, environments, or situations in which this information might be wanted or relevant again (e.g. ["home maintenance", "safety", "household", "storage", "reference"]). NEVER return an empty array [].
...
5. ABSOLUTE DATE/TIME NORMALISATION (WHEN TEMPORAL INTENT IS PRESENT):
   - When a memory contains relative time or natural language scheduling (e.g. "in 10 minutes", "tomorrow morning at 4am", "Saturday morning at 9am", "Monday evening at 7pm", "Dentist Tuesday at 2pm"), calculate the exact target date & time using the User Local Date & Time and Timezone provided in the Reference Context.
...
6. STRICT STRUCTURED OUTPUT:
  ...
  // Extract memories array from the structured JSON response
  let memoryItems: any[] = [];
  if (Array.isArray(structuredData)) {
    memoryItems = structuredData;
  } else if (structuredData && Array.isArray(structuredData.memories) && structuredData.memories.length > 0) {
    memoryItems = structuredData.memories;
  } else if (structuredData && typeof structuredData === 'object' && structuredData.content) {
    memoryItems = [structuredData];
  } else {
    memoryItems = [fallbackInterpretation(trimmedText, localContext.referenceDate)];
  }

  // Insert every memory object as its own separate record into persistent Bunny Database
  const now = new Date().toISOString();
  const newMemories = memoryItems.map((item, index) => {
    // Validate that original_time_expression only captures explicit user temporal wording, not inferred situational phrases
    ...
    if (cleanOriginalTime) {
      const isExplicitInText = trimmedText.toLowerCase().includes(cleanOriginalTime.toLowerCase());
      const isSituationalClause = /^(when|if|whenever|in case|after)\s+/i.test(cleanOriginalTime) && !/\b(\d+|today|tomorrow|yesterday|morning|afternoon|evening|night|am|pm|monday|tuesday|wednesday|thursday|friday|saturday|sunday|week|month|year|minute|hour|sec|o'clock)\b/i.test(cleanOriginalTime);
    ...
    if (retrievalCues.length === 0) {
      const words = trimmedText.split(/\s+/).map((w: string) => w.replace(/[^a-zA-Z0-9]/g, '').toLowerCase()).filter((w: string) => w.length > 2);
      retrievalCues = Array.from(new Set([item.content || trimmedText, ...(Array.isArray(item.topics) ? item.topics : []), ...words])).slice(0, 6);
    }
    ...
    const interpretation = {
      content: item.content || trimmedText,
